...
let BuscarFilmeComponent = class BuscarFilmeComponent {
    constructor(http, liveAnnouncer) {
        this.http = http;
...
BuscarFilmeComponent.ctorParameters = () => [
...
let DetalhesFilmeComponent = class DetalhesFilmeComponent {
    constructor(movieService, dialogRef, data // cria uma variavel que traz os dados que vão compor o dialog; Componente que o chama, informa os dados. (Exemplo: id do filme, tamanho da imagem para renderizar)
    ) {
    ...
    getDetail(movieId) {
        this.movieSub = this.movieService.getById(movieId).subscribe(movie => { this.movieDetail = movie; }, erro => { });
    }
};
DetalhesFilmeComponent.ctorParameters = () => [
...
let IntroComponent = class IntroComponent {
    constructor(liveAnnouncer, router) {
        this.liveAnnouncer = liveAnnouncer;
        this.router = router;
...
let ListagemFilmesComponent = class ListagemFilmesComponent {
    constructor(httpService, dialog, breakpointObserver, liveAnnouncer) {
    ...
    openDialog(movieId) {
        ...
        const dialogRef = this.dialog.open(_detalhes_filme_detalhes_filme_component__WEBPACK_IMPORTED_MODULE_4__["DetalhesFilmeComponent"], {
            width: '600px',
...
let SeletorGeneroComponent = class SeletorGeneroComponent {
    ...
    getGenreById(value) {
        this.generoService.loadGenreById(value).subscribe(dados => {
            this.result.total_pages = dados.total_pages;
            this.result.total_results = dados.total_results;
            this.result.page = dados.page;
            this.notifyChange();
...
SeletorGeneroComponent.ctorParameters = () => [
